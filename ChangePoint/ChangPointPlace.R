...
data_org=data_org %>% arrange(Date) %>% filter(!(Date%in%ymd(20160101,20170101)))
...
## 爆發流動性危機
tmp_time = ymd(20080111)# 發現有改變點的時間
...
data.frame(data$date[data$CUSUM_s>=1],detech_time)
...
plot(test$Date,test$Value,type="l")
points(test$Date[test$Date%in%detech_time],test$Value[test$Date%in%detech_time],col="red")
abline(v=ymd(20200330))
